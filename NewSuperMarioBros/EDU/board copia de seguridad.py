...
class Board:
    def __init__(self, x: int, y: int, fps: int):
        ...
        pyxel.init(x, y, title='New Super Mario Bros', fps=fps)
        ...
        self.enemy_list = self.enemies.enemy_list
        self.e = KoopaTroopa(255, 10)
    ...
    def draw(self):
        if self.player.lives > 0 and self.player.x < 2000:
            ...
            # for i in self.enemies.enemy_list:
            pyxel.blt(self.e.x - self.player.scroll_x, self.e.y, *self.e.sprite)
    ...
    """ def enemy_generate(self):
         if pyxel.frame_count / 240 == 0:
             if random.random() >= 0.75:
                 a = Goomba(255, 0)
                 self.enemies.enemy_list.append(a)
             elif random.random() <= 0.25:
                 a = KoopaTroopa(255, 0)
                 self.enemies.enemy_list.append(a)"""

    def check_for_floor(self, x, y):
        tile = self.get_tilemap(x // 8, y // 8)
        for k in self.blocks.col_tiles:
            return tile == k
    ...
    def update(self):
        ...
        """for i in self.enemies.enemy_list:
            i.update()
            if self.check_for_floor(i.x, i.y + 8) or self.check_for_floor(i.x + 7, i.y + 8):
                if i.is_falling:
                    i.is_falling = False
                    if i.x < i.x:
                        i.direction = -1
                        ...
                        i.direction = 1
                elif i.direction > 0 and self.check_for_floor(i.x + 8, i.y + 4):
                    i.direction = -1
            else:
                i.is_falling = True

            i.x, i.y, i.dx, i.dy = self.react_on_collision(i.x, i.y, i.dx, i.dy)"""

        if self.check_for_floor(self.e.x, self.e.y + 8) or self.check_for_floor(self.e.x + 7, self.e.y + 8):
            if self.e.is_falling:
                self.e.is_falling = False
                if self.e.x < self.e.x:
                    self.e.direction = -1
                else:
                    self.e.direction = 1
            elif self.e.direction > 0 and self.check_for_floor(i.x + 8, i.y + 4):
                self.e.direction = -1
        else:
            self.e.is_falling = True

        self.e.x, self.e.y, self.e.dx, self.e.dy = self.react_on_collision(self.e.x, self.e.y, self.e.dx, self.e.dy)
        ...
        if self.player.y > 300:
            self.game_ends()
""""
    def questionblock_function(self):
        question_block = QuestionBlocks()
        if self.player.x == question_block.x and self.player.y + 16 == question_block.y:
            pyxel.btl(question_block.x, question_block.y, 0, 16, 16)
            if question_block.container == "mushroom":
                self.blocks.mushroom_comes()
            if question_block.container == "coin":
                self.blocks.coin_comer()
"""