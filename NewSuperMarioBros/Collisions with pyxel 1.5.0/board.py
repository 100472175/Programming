...
class Board:
    def __init__(self, x: int, y: int, fps: int, time: int):
        ...
        self.blocks = Blocks()
        self.cooldown = 0
        self.mushroom_cooldown = 0
        self.enemy_cooldown = 0
        self.coin_cooldown = 0
        self.level = 0
        self.col_text = 0
    ...
    def check_teleport_collision(self, x, y):
        x1 = x // 8
        y1 = y // 8
        x2 = (x + self.player.width - 1) // 8
        y2 = (y + self.player.height - 1) // 8
        for i in range(y1, y2 + 1):
            for j in range(x1, x2 + 1):
                if self.level == 1:
                    for k in self.blocks.teleport_tile:
                        if self.get_tilemap(j, i) == k:
                            return True
        return False

    def coin_collision(self, x, y):
        x1 = x // 8
        y1 = y // 8
        x2 = (x + self.player.width - 1) // 8
        y2 = (y + self.player.height - 1) // 8
        for i in range(y1, y2 + 1):
            for j in range(x1, x2 + 1):
                if self.level == 0:
                    for k in self.blocks.coins_tiles:
                        if self.get_tilemap(j, i) == k:
                            return True
                elif self.level == 1:
                    for k in self.blocks.underworld_coins_tiles:
                        if self.get_tilemap(j, i) == k:
                            return True
        return False
    ...
    def draw(self):
        ...
        if self.player.lives > 0 and self.player.x < 2000:
            ...
            # pyxel.text(50, 50, "Scroll_x" + str(self.player.scroll_x), self.col_text)
        ...
        elif (self.player.lives == 0 or self.player.y > 300) or self.player.y > 250:
            pyxel.cls(0)
    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.KEY_W) or pyxel.btnp(pyxel.KEY_UP) or pyxel.btnp(pyxel.KEY_SPACE):
        ...
        if self.check_teleport_collision(self.player.x, self.player.y):
            self.level = 0
            self.teleport_to_overworld()

        if self.coin_collision(self.player.x, self.player.y):
            if self.coin_cooldown <= 0:
                self.player.coins += 1
                self.player.score += 200
                self.coin_cooldown = 1

    def teleport_to_overworld(self):
        self.level = 0
        self.col_text = 0
        self.player.scroll_x = 1385
        self.player.x = 174*8 - self.player.scroll_x
        self.player.y = 80
