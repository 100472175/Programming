...
def draw():
    ...
    # To draw a line: .line(startx: int, starty: int, endx: int, endy: int)
    x = pyxel.frame_count % pyxel.width
    pyxel.line(x, x, 20, 20, 3)
    # For a rectangle: .rect(x: int, y: int, w: int, h: int, col: int)
    # For a rectangle: .rect(left_corner_x,: int, left_corner_y: int, width: int, height: int, color: int)
    pyxel.rect(20, 20, 10, 30, 4)
    # For the 'frame' of a rectangle, same as the rectangle
